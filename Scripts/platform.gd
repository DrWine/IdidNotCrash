...
func _ready() -> void:
	...
	if !WandS:
		dirs[Vector2(-1, 0)] = (markers["left"].global_position - global_position).normalized()
		dirs[Vector2(1, 0)] = (markers["right"].global_position - global_position).normalized()
	else:
...
func get_limits(player_position: Vector2, axis: Vector2) -> int:
	...
	if !WandS:
		if player_position.x < markers["left"].global_position.x - safe_margin:
			if axis.x < 0:
				return 0
			return 1
		if player_position.x > markers["right"].global_position.x + safe_margin:
			if axis.x > 0:
				return 0
			return 1
	else:
